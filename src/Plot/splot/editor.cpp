...
{
    d_editedItem = static_cast<InteractiveShapeItem*>(itemAt( pos ));
    if ( d_editedItem )
    {
...
{
    const QwtPlot *plot = this->plot();
    if ( plot == nullptr || d_editedItem == nullptr )
        return;
